...
function load() {
  ...
  return { version: 1, items: [], settings: { tmdbKey: '', compact: false } };
}
...
function bumpProgress(id, dS, dE) {
  const it = state.items.find(i => i.id === id);
  if (!it) return;
  let s = it.tvS || 1, e = it.tvE || 0;
  if (dE) {
    e = Math.max(0, e + dE);
  }
  if (dS) {
  ...
  patch(id, { tvS: s, tvE: e, status: it.status === 'want' ? 'active' : it.status });
}
...
const epLabel = (it) => it.tvS ? `S${it.tvS}·E${it.tvE || 0}` : '';
const haystack = (it) => [it.title, it.creator, it.year, it.recommendedBy, it.notes]
  .join(' ').toLowerCase().replace(/"/g, '');

function cardHtml(it) {
  const sub = [it.creator, it.year].filter(Boolean).join(' · ');
  const isTvActive = it.type === 'tv' && it.status === 'active';
  if (state.settings.compact) {
    return `<article class="card compact" data-open="${it.id}" data-s="${esc(haystack(it))}">
      <span class="ctype">${TYPES[it.type].icon}</span>
      <div class="body">
        <div class="title">${esc(it.title)}</div>
        <div class="sub">${esc([sub, it.recommendedBy ? 'via ' + it.recommendedBy : ''].filter(Boolean).join(' · '))}</div>
      </div>
      ${it.rating ? `<span class="crate">${it.rating}★</span>`
        : isTvActive && it.tvS ? `<span class="cstatus">${epLabel(it)}</span>`
        : `<span class="cstatus">${esc(statusLabel(it))}</span>`}
    </article>`;
  }
  return `<article class="card" data-open="${it.id}" data-s="${esc(haystack(it))}">
    ${coverHtml(it)}
    <div class="body">
      <div class="title">${esc(it.title)}</div>
      ...
      <div class="row">
        ...
        ${isTvActive && it.tvS ? `<span class="pill">${epLabel(it)}</span>` : ''}
        ${it.rating ? starsHtml(it.rating, it.id) : ''}
        ${isTvActive ? `<button class="epbtn" data-ep="${it.id}">+1 ep</button>` : ''}
      </div>
    </div>
  </article>`;
}
...
function render() {
  ...
  dens.hidden = !(view === 'queue' || view === 'library');
  dens.classList.toggle('on', state.settings.compact);
...
function renderLibrary() {
  ...
  app.innerHTML =
    `<div class="field"><input id="libSearch" placeholder="Search your library…" autocomplete="off"
      value="${esc(libQuery)}" enterkeyhint="search"></div>` +
    `<div class="filters">${chips.map(([k, l]) =>
      `<button class="chip ${libFilter === k ? 'on' : ''}" data-filter="${k}">${l}</button>`).join('')}</div>` +
    `<div id="libList">${items.length ? items.map(cardHtml).join('')
...
function applyLibQuery() {
  ...
  let shown = 0;
  document.querySelectorAll('#libList .card').forEach(c => {
    const hit = !q || (c.dataset.s || '').includes(q);
    c.hidden = !hit;
    if (hit) shown++;
  });
  const empty = document.querySelector('#libList .empty');
  if (empty) empty.hidden = !!q;
  let none = document.getElementById('libNone');
  if (q && !shown) {
    if (!none) {
      none = document.createElement('div');
      none.id = 'libNone'; none.className = 'empty';
      none.textContent = 'No matches in your library.';
      document.getElementById('libList').appendChild(none);
    }
    none.hidden = false;
...
function renderAdd() {
  ...
  app.innerHTML = `
    ...
    <div class="row" style="margin-top:10px">
      ${(addType === 'book' || addType === 'audiobook')
        ? `<button class="btn secondary sm" id="scanBtn">📷 Scan barcode</button>` : ''}
      <button class="btn secondary sm" id="manualBtn">Add manually</button>
    </div>
...
function closeSheet() { stopScan(); modal.hidden = true; modalSheet.innerHTML = ''; }
...
function openDetail(id) {
  ...
  openSheet(`
    ...
    ${it.type === 'tv' ? `
    <div class="field"><label>Up to${it.seasons ? ' <span class="muted">(' + it.seasons + ' seasons)</span>' : ''}</label>
      <div class="stepper" id="dProg">
        <span class="sbox">Season <b id="pS">${it.tvS || 1}</b></span>
        <button data-p="s-" class="sbtn">−</button><button data-p="s+" class="sbtn">＋</button>
        <span class="sbox">Ep <b id="pE">${it.tvE || 0}</b></span>
        <button data-p="e-" class="sbtn">−</button><button data-p="e+" class="sbtn">＋</button>
      </div>
    </div>` : ''}
  ...
  const prog = modalSheet.querySelector('#dProg');
  if (prog) prog.addEventListener('click', e => {
    const b = e.target.closest('[data-p]'); if (!b) return;
    const map = { 's-': [-1, 0], 's+': [1, 0], 'e-': [0, -1], 'e+': [0, 1] }[b.dataset.p];
    bumpProgress(id, map[0], map[1]);
    const it = state.items.find(i => i.id === id);
    modalSheet.querySelector('#pS').textContent = it.tvS || 1;
    modalSheet.querySelector('#pE').textContent = it.tvE || 0;
    refreshBg();
  });
...
function openManual(prefill = {}) {
  const t = TYPES[addType];
  const creatorLabel = addType === 'movie' ? 'Director' : addType === 'tv' ? 'Creator' : 'Author';
  openSheet(`
    <h2>Add ${esc(t.label.toLowerCase())} manually</h2>
    <div class="field"><label>Title</label><input id="mTitle" value="${esc(prefill.title || '')}"></div>
    <div class="field"><label>${creatorLabel}</label><input id="mCreator" value="${esc(prefill.creator || '')}"></div>
    <div class="field"><label>Year</label><input id="mYear" inputmode="numeric" value="${esc(prefill.year || '')}"></div>
    <div class="field"><label>Description</label><textarea id="mDesc" rows="3">${esc(prefill.description || '')}</textarea></div>
    <div class="field"><label>Cover image URL <span class="muted">(optional)</span></label>
      <input id="mCover" value="${esc(prefill.cover || '')}" placeholder="https://…"></div>
    <div class="field"><label>Recommended by <span class="muted">(optional)</span></label>
      <input id="mRec" value="${esc(prefill.recommendedBy || '')}"></div>
    <button class="btn" id="mSave">Add to queue</button>`);
  modalSheet.querySelector('#mSave').onclick = () => {
    const title = modalSheet.querySelector('#mTitle').value.trim();
    if (!title) { alert('Please enter a title.'); return; }
    const found = {
      type: addType,
      source: {
        provider: (prefill.source && prefill.source.provider) || 'manual',
        id: (prefill.source && prefill.source.id) || ('m' + uid()),
      },
      title,
      creator: modalSheet.querySelector('#mCreator').value.trim(),
      year: modalSheet.querySelector('#mYear').value.trim(),
      description: modalSheet.querySelector('#mDesc').value.trim(),
      cover: modalSheet.querySelector('#mCover').value.trim(),
      link: prefill.link || '',
      isbn: prefill.isbn,
    };
    const res = addItem(found, modalSheet.querySelector('#mRec').value.trim());
    closeSheet();
    if (res.dup) alert('That’s already in your list.');
    view = 'queue'; render();
  };
}

/* ---------- barcode scan ---------- */
let scanReader = null, scanStream = null;
function stopScan() {
...
async function lookupIsbn(isbn) {
  const type = addType === 'audiobook' ? 'audiobook' : 'book';
  try {
    const r = await fetch(`https://openlibrary.org/api/books?bibkeys=ISBN:${isbn}&format=json&jscmd=data`);
    const b = (await r.json())['ISBN:' + isbn];
    if (b) return {
      type, source: { provider: 'isbn', id: isbn }, isbn,
      title: b.title || '',
      creator: (b.authors || []).map(a => a.name).join(', '),
      year: (b.publish_date || '').match(/\d{4}/)?.[0] || '',
      description: (b.notes && (b.notes.value || b.notes)) || '',
      cover: (b.cover && (b.cover.medium || b.cover.large || b.cover.small)) || '',
      link: b.url || '',
    };
  } catch (e) {}
  try {
    const vi = (await (await fetch('https://www.googleapis.com/books/v1/volumes?q=isbn:' + isbn))
    ...
    if (vi) return {
      ...
      year: (vi.publishedDate || '').slice(0, 4),
      description: vi.description || '',
      cover: ((vi.imageLinks && vi.imageLinks.thumbnail) || '').replace('http://', 'https://'),
      link: vi.infoLink || '',
    };
  } catch (e) {}
  return null;
...
async function openScan() {
  openSheet(`
    ...
    <p class="muted" id="scanMsg">Point the camera at the barcode on the back of the book.</p>
    <video id="scanVid" playsinline muted
      style="width:100%;border-radius:12px;background:#000;aspect-ratio:3/2;object-fit:cover"></video>
    <button class="btn secondary" id="scanCancel" style="margin-top:12px">Cancel</button>`);
  const msg = modalSheet.querySelector('#scanMsg');
  const vid = modalSheet.querySelector('#scanVid');
  modalSheet.querySelector('#scanCancel').onclick = closeSheet;

  async function onCode(raw) {
    const isbn = String(raw).replace(/[^0-9Xx]/g, '');
    if (isbn.length !== 10 && isbn.length !== 13) return;
    stopScan();
    openSheet(`<div class="spinner">Looking up ${esc(isbn)}…</div>`);
    let found = null;
    try { found = await lookupIsbn(isbn); } catch (e) {}
    openManual(found || { isbn, source: { provider: 'isbn', id: isbn } });
  }
  ...
  try {
    ...
    if ('BarcodeDetector' in window) {
      ...
      const tick = async () => {
        ...
        try { const c = await det.detect(vid); if (c[0]) return onCode(c[0].rawValue); } catch (e) {}
      ...
      scanReader = new window.ZXing.BrowserMultiFormatReader();
      scanReader.decodeFromVideoElement(vid, (result) => { if (result) onCode(result.getText()); });
    }
  } catch (e) {
    stopScan();
    if (msg) msg.textContent = 'Camera not available (' + e.message + '). Add the book manually instead.';
  }
}
...
document.getElementById('btnDensity').onclick = () => {
...
app.addEventListener('click', e => {
  ...
  if (ep) { bumpProgress(ep.dataset.ep, 0, 1); render(); return; }
  ...
  if (e.target.id === 'manualBtn') { openManual(); return; }
  if (e.target.id === 'scanBtn') { openScan(); return; }
...
app.addEventListener('input', e => {
  if (e.target.id === 'libSearch') { libQuery = e.target.value; applyLibQuery(); }
});
